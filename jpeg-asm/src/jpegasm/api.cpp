...
#include "../../deps/stb_image_resize/stb_image_resize.cpp"
...
*/
    ...
    function("stbir_resize_uint8", optional_override([](const std::string input_pixels, int input_w, int input_h, int input_stride_in_bytes,
        uintptr_t output_pixels, int output_w, int output_h, int output_stride_in_bytes, int num_channels
    ){
        const unsigned char * inputUC = reinterpret_cast<const unsigned char *>(input_pixels.c_str());
        unsigned char * outputUC = reinterpret_cast<unsigned char *>(output_pixels);
        return stbir_resize_uint8(inputUC, input_w, input_h, input_stride_in_bytes, outputUC, output_w, output_h, output_stride_in_bytes, num_channels);
    }));
...
int stbir_resize_uint8_generic( const unsigned char *input_pixels , int input_w , int input_h , int input_stride_in_bytes,
                                         ...
                                         void *alloc_context);
*/
    function("stbir_resize_uint8_generic", optional_override([](const std::string input_pixels, int input_w, int input_h, int input_stride_in_bytes,
        uintptr_t output_pixels, int output_w, int output_h, int output_stride_in_bytes, int num_channels,
         int filter // can be modified, 0 is default ("mitchell")
    ){
        const unsigned char * inputUC = reinterpret_cast<const unsigned char *>(input_pixels.c_str());
        unsigned char * outputUC = reinterpret_cast<unsigned char *>(output_pixels);
        return stbir_resize_uint8_generic(inputUC, input_w, input_h, input_stride_in_bytes, outputUC, output_w, output_h, output_stride_in_bytes, num_channels,
        -1,
        0,
        (stbir_edge)1,
        (stbir_filter)filter,
