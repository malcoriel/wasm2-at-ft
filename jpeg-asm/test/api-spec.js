...
var readBigSample = function () {
    var jpegBuffer = fs.readFileSync(__dirname + '/data/camera.jpg');
...
describe('JpegAsm', function () {
    ...
    describe('High-Level API', function () {
        ...
        it('dec-enc', function () {
            this.timeout(30000);
            var jpegArray = readBigSample();

            var decodedObj = api.decode(jpegArray);

            decodedObj.buffer.byteLength.should.be.greaterThan(0);
            decodedObj.width.should.be.equal(5760);
            decodedObj.height.should.be.equal(3840);


            var quality = 80;

            var encoded = api.encode(decodedObj.buffer, 1024, 768, quality);
            encoded.byteLength.should.be.greaterThan(0);
            saveFile(encoded, 'camera-recoded.jpg');
        });
        ...
        it('downsample', function () {
            ...
            var resized = api.resize(decodedObj.buffer, decodedObj.width, decodedObj.height, targetWidth, targetHeight, "box");
            var encoded = api.encode(resized, targetWidth, targetHeight, 100);
            saveFile(encoded, 'camera-downsampled.jpg');
        });
    });
});
