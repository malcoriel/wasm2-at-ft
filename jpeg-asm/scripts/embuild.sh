...
function jpegasm_build {
  ...
  if [[ $DEBUG -eq 1 ]]; then
    PRE_POST=
    CFLAGS="-std=c++11 -s ALLOW_MEMORY_GROWTH=1"
  else
    PRE_POST=
    CFLAGS="-std=c++11 -O3 -s ALLOW_MEMORY_GROWTH=1 --memory-init-file 0"
  fi
...
set -x
  ...
  cd ${ROOT_DIR}/build
  ${EMCC} ${CFLAGS} -Wl,-l${JPEG_SO_PATH} ${SRC_DIR}/api.cpp -I../deps/${JPEG_NAME} -o lib${LIBNAME}.bc
  ${EMCC} ${CFLAGS} ${PRE_POST} ${JPEG_SO_PATH} lib${LIBNAME}.bc --bind -s WASM=1 -s EXPORTED_FUNCTIONS=@../scripts/exported_functions --pre-js ../scripts/pre.js -o lib${LIBNAME}.js
set +x
